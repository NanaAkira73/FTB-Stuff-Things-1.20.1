...
public class RecipeMultiCache<R extends Recipe<?>> {
    ...
    private final Int2ObjectLinkedOpenHashMap<List<R>> recipeCache = new Int2ObjectLinkedOpenHashMap<>(MAX_CACHE_SIZE, 0.25f);
     ...
     */
    public List<R> getCachedRecipes(Supplier<List<R>> recipeFinder, IntSupplier hashCodeGenerator) {
        int key = hashCodeGenerator.getAsInt();

        if (recipeCache.containsKey(key)) {
            return recipeCache.getAndMoveToFirst(key);
        } else {
            List<R> newRecipes = recipeFinder.get();
