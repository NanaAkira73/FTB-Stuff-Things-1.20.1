...
public class FTBStuffJeiPlugin implements IModPlugin {
    ...
    public void registerRecipeCatalysts(IRecipeCatalystRegistration registration) {
        registration.addRecipeCatalyst(new ItemStack(ItemsRegistry.TEMPERED_JAR.get()), RecipeTypes.TEMPERED_JAR);
        registration.addRecipeCatalyst(new ItemStack(ItemsRegistry.CREATIVE_HOT_TEMPERATURE_SOURCE.get()), RecipeTypes.TEMPERATURE_SOURCE);
        registration.addRecipeCatalyst(new ItemStack(ItemsRegistry.CREATIVE_SUPERHEATED_TEMPERATURE_SOURCE.get()), RecipeTypes.TEMPERATURE_SOURCE);
        registration.addRecipeCatalyst(new ItemStack(ItemsRegistry.CREATIVE_CHILLED_TEMPERATURE_SOURCE.get()), RecipeTypes.TEMPERATURE_SOURCE);
        registration.addRecipeCatalyst(new ItemStack(ItemsRegistry.CROOK.get()), RecipeTypes.CROOK);
        registration.addRecipeCatalyst(new ItemStack(ItemsRegistry.DRIPPER.get()), RecipeTypes.DRIPPER);
        registration.addRecipeCatalyst(new ItemStack(ItemsRegistry.FUSING_MACHINE.get()), RecipeTypes.FUSING_MACHINE);
        registration.addRecipeCatalyst(new ItemStack(ItemsRegistry.SUPER_COOLER.get()), RecipeTypes.SUPER_COOLER);
        registration.addRecipeCatalyst(new ItemStack(ItemsRegistry.WOODEN_BASIN.get()), RecipeTypes.WOODEN_BASIN);

        for (var item : ItemsRegistry.ALL_HAMMERS) {
            registration.addRecipeCatalyst(new ItemStack(item.get()), RecipeTypes.HAMMER);
        }
        for (var block : BlocksRegistry.ALL_AUTO_HAMMERS) {
            registration.addRecipeCatalyst(new ItemStack(block.get()), RecipeTypes.HAMMER);
        }
        for (var block : BlocksRegistry.ALL_SLUICES) {
            registration.addRecipeCatalyst(new ItemStack(block.get()), RecipeTypes.SLUICE);
        }

//        BlocksRegistry.waterStrainers().forEach(b -> registration.addRecipeCatalyst(new ItemStack(b.get()), RecipeTypes.LOOT_SUMMARY));
    }
    ...
    private <I extends Container, T extends Recipe<I>> void addRecipeType(IRecipeRegistration registration, net.minecraft.world.item.crafting.RecipeType<T> mcRecipeType, RecipeType<T> jeiRecipeType, Function<List<T>, List<T>> postProcessor) {
        List<T> recipes = Minecraft.getInstance().level.getRecipeManager().getAllRecipesFor(mcRecipeType).stream()
                .filter(r -> r instanceof IHideableRecipe ihr && ihr.shouldShowRecipe())
                .map(r -> (T) r)
                .toList();
